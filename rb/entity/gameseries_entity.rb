...
class GameseriesEntity
  ...
  def make
    opts = @_entopts.dup
    GameseriesEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AmiiboapiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Gameseries, Hash] the current Gameseries data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = AmiiboapiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Gameseries fields)
  def match_get
  ...
  #
  # @param reqmatch [GameseriesListMatch, Hash, nil] match filter (any subset of
  #   Gameseries fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Gameseries>, Array] the matching Gameseries items; raises AmiiboapiError on failure

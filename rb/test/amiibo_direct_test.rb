...
class AmiiboDirectTest < Minitest::Test
  def test_direct_list_amiibo
    setup = amiibo_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-amiibo", setup[:live] ? "live" : "unit")
    if _should_skip
...
def amiibo_direct_setup(mockres)

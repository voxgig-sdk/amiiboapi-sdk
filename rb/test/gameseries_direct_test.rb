...
class GameseriesDirectTest < Minitest::Test
  def test_direct_list_gameseries
    setup = gameseries_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-gameseries", setup[:live] ? "live" : "unit")
    if _should_skip
...
def gameseries_direct_setup(mockres)

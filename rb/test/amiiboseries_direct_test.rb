...
class AmiiboseriesDirectTest < Minitest::Test
  def test_direct_list_amiiboseries
    setup = amiiboseries_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-amiiboseries", setup[:live] ? "live" : "unit")
    if _should_skip
...
def amiiboseries_direct_setup(mockres)

...
// Amiiboapi SDK Gameseries entity
...
class GameseriesEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GameseriesEntity($this->_client, $opts);
    }

    /**
     * @param Gameseries|array $args Gameseries data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = AmiiboapiHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Gameseries|array The current Gameseries data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Gameseries fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = AmiiboapiHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Gameseries fields).
     */
    ...
    /**
     * List Gameseries items matching the given filter.
     *
     * @param GameseriesListMatch|array|null $reqmatch Match filter (any subset
     *   of Gameseries fields) as an assoc-array; GameseriesListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Gameseries[]|array A list of Gameseries items as assoc-arrays at
     *   the SDK boundary; throws AmiiboapiError on failure (item-5 convention).
     */

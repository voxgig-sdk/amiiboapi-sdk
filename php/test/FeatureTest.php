...
class FtCtrl extends AmiiboapiControl
{
...
class FtClient
{
    ...
    private AmiiboapiUtility $utility;

    public function __construct(AmiiboapiUtility $utility, array $options)
    {
    ...
    public function get_utility(): AmiiboapiUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AmiiboapiContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AmiiboapiContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AmiiboapiContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AmiiboapiConfig::make_feature($name);
    ...
    private static function buildUrl(AmiiboapiSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AmiiboapiError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AmiiboapiResponse($fetched);
        }
    ...
    private function fail(AmiiboapiContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AmiiboapiContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

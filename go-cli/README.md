...
boru-driven command-line client **and** interactive REPL for the Amiiboapi
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/amiiboapi-cli)
make build
...
./amiiboapi-cli list amiibo
./amiiboapi-cli list amiiboseries

# 5. Override the API base URL for a single call
AMIIBOAPI_BASE=https://api.example.com ./amiiboapi-cli list amiibo

# 6. No arguments -> interactive REPL
./amiiboapi-cli
amiiboapi> list amiibo
amiiboapi> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/amiiboapi-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/amiiboapi-cli list amiibo
   ```
...
Run with no arguments to open a REPL (prompt `amiiboapi>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `amiibo`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/amiiboapi-cli`. |
...
(`./amiiboapi-cli <expr>`) and interactively (the REPL), and expressions compose the

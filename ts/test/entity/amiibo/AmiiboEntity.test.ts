...
describe('AmiiboEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'amiibo.' + op, live)) return
    }
    ...
    const amiibo_ref01_list = await amiibo_ref01_ent.list(amiibo_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['AMIIBOAPI_TEST_AMIIBO_ENTID']

...
//     (\`new AmiiboapiSDK(...)\` / \`AmiiboapiSDK.test(...)\` become
//     \`AmiiboapiSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"amiibo":{"test01":{"id":"test01"}},"amiiboseries":{"test01":{"id":"test01"}},"character":{"test01":{"id":"test01"}},"gameseries":{"test01":{"id":"test01"}},"type":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AmiiboapiSDK(...)\` and any
// \`AmiiboapiSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AmiiboapiSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AmiiboapiSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

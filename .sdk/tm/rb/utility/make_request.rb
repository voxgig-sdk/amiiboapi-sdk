...
module AmiiboapiUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AmiiboapiResponse.new({})
    ...
    if fetch_err
      ...
      response = AmiiboapiResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AmiiboapiResponse.new(fetched)
    else

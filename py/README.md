...
The Python SDK for the Amiiboapi API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Amiibo(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.Amiiboapi(None).load(
    {"id": "test01"}, None
)

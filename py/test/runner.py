...
class AmiiboapiTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            AmiiboapiTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = AmiiboapiTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = AmiiboapiTestRunner.getenv("AMIIBOAPI_TEST_LIVE")
        override = AmiiboapiTestRunner.getenv("AMIIBOAPI_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = AmiiboapiTestRunner.getenv(key)
        ...
        explain = AmiiboapiTestRunner.getenv("AMIIBOAPI_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["AMIIBOAPI_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if AmiiboapiTestRunner._test_control is not None:
            return AmiiboapiTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                AmiiboapiTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            AmiiboapiTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = AmiiboapiTestRunner.load_test_control()
...
def load_env_local():
    AmiiboapiTestRunner.load_env_local()


def env_override(m):
    return AmiiboapiTestRunner.env_override(m)


def entity_data(v):
    return AmiiboapiTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return AmiiboapiTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return AmiiboapiTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return AmiiboapiTestRunner.load_test_control()


def live_client_options():
    return AmiiboapiTestRunner.live_client_options()


def live_delay_ms():
    return AmiiboapiTestRunner.live_delay_ms()

...
//
// NewAmiiboapiSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.AmiiboapiSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewAmiiboapiSDK(opts)
}
...
func fcCandidates(client *sdk.AmiiboapiSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.AmiiboapiSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.AmiiboapiSDK, name string) any {

...
import (
	...
	omni "github.com/voxgig-sdk/amiiboapi-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AmiiboapiError); is {
				out := map[string]any{
					"name":    "AmiiboapiError",
...
func omniCtx(arg any, client *sdk.AmiiboapiSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AmiiboapiSDK); is {
			client = live

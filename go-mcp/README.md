...
[MCP](https://modelcontextprotocol.io) server exposing the Amiiboapi SDK as
two agent tools — `amiiboapi_list` and `amiiboapi_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/amiiboapi-mcp)
make build

# 2. Provide credentials via the environment
export AMIIBOAPI_APIKEY=sk_live_xxx
...
```jsonc
// amiiboapi_list: first page of records
{ "entity": "amiibo" }
{ "entity": "amiibo", "query": { } }

// amiiboapi_load: one record by id
{ "entity": "amiibo", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/amiiboapi-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user amiiboapi \
     -- "$PWD"/dist/*/amiiboapi-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `amiiboapi_list` and `amiiboapi_load` tools now appear
   in new sessions. Ask the agent to *"list amiibo using amiiboapi"*
   and it calls `amiiboapi_list` with `{"entity":"amiibo"}`.
...
```sh
export AMIIBOAPI_APIKEY=sk_live_xxx            # API key
export AMIIBOAPI_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `amiiboapi_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `amiiboapi_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `AMIIBOAPI_APIKEY` | API key sent with every request. |
| `AMIIBOAPI_BASE` | Optional override of the API base URL. |

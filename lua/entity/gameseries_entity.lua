...
local GameseriesEntity = {}
GameseriesEntity.__index = GameseriesEntity


function GameseriesEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GameseriesEntity)
  self._name = "gameseries"
  self._client = client
...
function GameseriesEntity:get_name()
  return self._name
end


function GameseriesEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GameseriesEntity.new(self._client, opts)
end
...
function GameseriesEntity:mark_deleted()
  self._deleted = true
end


function GameseriesEntity:deleted()
  return true == self._deleted
end


function GameseriesEntity:data_set(args)
  if args ~= nil then
...
function GameseriesEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GameseriesEntity:match_set(args)
  if args ~= nil then
...
function GameseriesEntity:match_get()
...
function GameseriesEntity:stream(action, args, callopts)
...
function GameseriesEntity:_run_op(ctx, post_done)
  local utility = self._utility

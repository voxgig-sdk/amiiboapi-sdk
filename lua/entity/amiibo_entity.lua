...
local AmiiboEntity = {}
AmiiboEntity.__index = AmiiboEntity


function AmiiboEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AmiiboEntity)
  self._name = "amiibo"
  self._client = client
...
function AmiiboEntity:get_name()
  return self._name
end


function AmiiboEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AmiiboEntity.new(self._client, opts)
end
...
function AmiiboEntity:mark_deleted()
  self._deleted = true
end


function AmiiboEntity:deleted()
  return true == self._deleted
end


function AmiiboEntity:data_set(args)
  if args ~= nil then
...
function AmiiboEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AmiiboEntity:match_set(args)
  if args ~= nil then
...
function AmiiboEntity:match_get()
...
function AmiiboEntity:stream(action, args, callopts)
...
function AmiiboEntity:_run_op(ctx, post_done)
  local utility = self._utility

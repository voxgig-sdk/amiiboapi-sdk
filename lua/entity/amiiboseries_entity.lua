...
local AmiiboseriesEntity = {}
AmiiboseriesEntity.__index = AmiiboseriesEntity


function AmiiboseriesEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AmiiboseriesEntity)
  self._name = "amiiboseries"
  self._client = client
...
function AmiiboseriesEntity:get_name()
  return self._name
end


function AmiiboseriesEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AmiiboseriesEntity.new(self._client, opts)
end
...
function AmiiboseriesEntity:mark_deleted()
  self._deleted = true
end


function AmiiboseriesEntity:deleted()
  return true == self._deleted
end


function AmiiboseriesEntity:data_set(args)
  if args ~= nil then
...
function AmiiboseriesEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AmiiboseriesEntity:match_set(args)
  if args ~= nil then
...
function AmiiboseriesEntity:match_get()
...
function AmiiboseriesEntity:stream(action, args, callopts)
...
---@param reqmatch AmiiboseriesListMatch
---@param ctrl? table
---@return Amiiboseries[]
---@return string? err
function AmiiboseriesEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function AmiiboseriesEntity:_run_op(ctx, post_done)
  local utility = self._utility

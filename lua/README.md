...
It exposes the API as capitalised, semantic **Entities** — e.g. `client:Amiibo()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.

...
The Lua SDK for the Amiiboapi API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Amiibo(nil):list(nil, nil)
...
local result, err = client:Amiiboapi(nil):load(
  { id = "test01" }, nil
)
